...
<script lang="ts">
    ...
    function keydown(ev: KeyboardEvent) {
        ...
        const saveScreenActive = typeSaveName === -2 || saveSearch;
        if (saveScreenActive && ev.key.toUpperCase() === 'L') {
            toggleSaveGamesSubmenu('load')();
            ev.preventDefault();
        }
        if (saveScreenActive && ev.key.toUpperCase() === 'S') {
        ...
        switch (ev.code) {
            case 'ArrowUp':
                saveGames.then(sg => cursor.update(n => wrapAround(n - 1, sg.length, cursorMin)));
                break;
            case 'ArrowDown':
                saveGames.then(sg => cursor.update(n => wrapAround(n + 1, sg.length, cursorMin)));
                break;
    ...
    $: saveGames = (subMenu === 'load' || subMenu === 'save') ? sgs.loadGames(loadGameSearchText.toUpperCase()) : Promise.resolve([] as SaveGame[]);
    function saveGame(name: string, id?: number) {
        ...
        console.log('doom-save', id, name, save);
    ...
    async function loadGame(save: SaveGame) {
        ...
        restoreGame.set(save.mapExport);
        resumeGame();
    }
    const deleteSave = async (id: number) => {
        await sgs.deleteGame(id)
        saveGames = sgs.loadGames(loadGameSearchText.toUpperCase());
        menuSounds.sfx.pistol();
...
>
    ...
    >
        ...
        {:else if subMenu === 'save'}
            <h2 class="flex justify-center sticky top-12 bg-base-200 z-20 md:hidden">Save</h2>
            <div class="flex flex-wrap gap-4 p-2 items-center justify-start">
                <button class="btn"
                ...
                <label class="input input-bordered flex items-center gap-2 text-sm">
                    ...
                    <input bind:this={saveSearch} type="text" class="grow" placeholder="Search"
                        bind:value={loadGameSearchText}
                    />
                </label>
            </div>
            {#await saveGames}
            ...
            {:then games}
                <ul class="saves flex flex-col gap-2 px-8 py-2">
                    {#if !deleteSaveMode}
                    <button
                    ...
                    >
                        ...
                        {:else}
                            <label class="input input-bordered flex items-center gap-2 text-sm">
                                <input type="text" class="grow" placeholder="Name (optional)"
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <div class="absolute flex justify-center align-middle text-secondary">
                                <label class="input input-bordered flex items-center gap-2 text-sm">
                                    <input type="text" class="grow" placeholder="Name (optional)"
                                        bind:value={saveGameName}
                                    />
...
{#snippet gameTile(save: SaveGame)}
    ...
    <div class="absolute bottom-2 right-2 p-2 items-start flex flex-col gap-2 bg-black rounded-lg text-secondary"
    ...
    <div class="absolute bottom-2 left-2 p-2 items-start flex flex-col gap-2 bg-black rounded-lg"
        style:--tw-bg-opacity={.5}
    >
        <div class="text-primary">{save.name}</div>
        <div class="flex items-end text-secondary">
